import { getOrDefault, maxBy, minBy, setOrUpdate, sum } from '../extensions';
  ...
  implements ReadonlyMap<K, number>, JsonSerializable<Record<string, number>>
{
  ...
  get(key: K): number {
    return getOrDefault(this.map, key, 0);
  }

  add(key: K, amount = 1): this {
    setOrUpdate(this.map, key, (existing?: number) => (existing ?? 0) + amount);
    return this;
  }

  addAll(keys: K[], amount = 1): this {
    keys.forEach((key: K) => this.add(key, amount));
    return this;
  }
